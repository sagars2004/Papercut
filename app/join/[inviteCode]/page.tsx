...
type InviteRoom = { id: string; name: string; duration_days: number; status: "lobby" | "active" | "complete"; created_by: string };
...
export default async function JoinInvitePage({ params }: JoinInvitePageProps) {
  ...
  const { data: roomData } = await supabase.from("rooms").select("id, name, duration_days, status, created_by").eq("invite_code", normalizedCode).maybeSingle();
